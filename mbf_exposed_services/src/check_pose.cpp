...
bool CheckPose::check_pose_cost_from_costmap_and_footprint(costmap_2d::Costmap2D* costmap_2d,
                                                           std::vector<geometry_msgs::Point> footprint,
                                                           mbf_msgs::CheckPose::Request& request,
                                                           mbf_msgs::CheckPose::Response& response)
{
  ...
  std::vector<mbf_costmap_nav::Cell> footprint_cells =
      mbf_costmap_nav::FootprintHelper::getFootprintCells(x, y, yaw, footprint, *costmap_2d, true);
  ...
  else
  {
    ...
    boost::unique_lock<costmap_2d::Costmap2D::mutex_t> lock(*(costmap_2d->getMutex()));
    ...
    {
      unsigned char cost = costmap_2d->getCost(footprint_cells[i].x, footprint_cells[i].y);
      switch (cost)
      {
