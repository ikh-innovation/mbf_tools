                                ...
                                cmap::Costmap2DROS* /*global_costmap*/, cmap::Costmap2DROS* local_costmap)
{
    ...
    private_nh.searchParam("aristos_behaviour/CHECKPOSE_MVBACK_ENABLE", param_tmp);
    ...
    private_nh.searchParam("aristos_behaviour/CHECKPOSE_MVBACK_LOOKAHEAD", param_tmp);
    ...
    if (mvback_safety_dist>backwards_transpose_dist)
    {
        ...
        ROS_WARN("CHECKPOSE_MVBACK_DIST is greater than CHECKPOSE_MVBACK_LOOKAHEAD. Setting to CHECKPOSE_MVBACK_DIST to 0.8 of CHECKPOSE_MVBACK_LOOKAHEAD value.");
    }
...
geometry_msgs::PoseStamped MoveBackRecoveryCostmap::getTransposedRobotPose(double distance) const
{
    geometry_msgs::PoseStamped p;
    local_costmap_->getRobotPose(p);
...
uint32_t MoveBackRecoveryCostmap::moveBack()
{
    ...
    while (double dist_diff = (step_back_length_ - getCurrentDiff(initialPose)) > 0.01 && ros::ok())
    {
        ...
        local_costmap_->getCostmap()->worldToMap(currentGMPose.x, currentGMPose.y, mx, my);
        ...
        if (cost-prev_cost>0 && cost>occupied_ths_crv)
        {
            ...
            {
                ROS_INFO("CHECKING FOR PREMATURE PREEMPTION:\n \tPoint Cost: %f \n \tMoving to higher costs: %d", cost, cost-prev_cost>0);
                // if point_cost check is "lethal", check current position with safety distance
                ...
                if (check_pose_srv.response.state == check_pose_srv.response.LETHAL || check_pose_srv.response.state == check_pose_srv.response.UNKNOWN || check_pose_srv.response.state == check_pose_srv.response.OUTSIDE)
                {
                    if(check_pose_srv.response.state == check_pose_srv.response.LETHAL)
                    {
                        ROS_ERROR("REJECTING RECOVERY MOTION: BLOCKED_PATH\n \tCheckPose state: %d ", check_pose_srv.response.state);
                        return mbf_msgs::ExePathResult::BLOCKED_PATH;
                    }
                    else
                    {
                        ROS_ERROR("REJECTING RECOVERY MOTION: MAP_ERROR\n \tCheckPose state: %d. \nPlease consider to check CHECKPOSE_MVBACK_DIST and CHECKPOSE_MVBACK_LOOKAHEAD params too", check_pose_srv.response.state);
        ...
        if (enable_check_pose)
        {
            ...
            geometry_msgs::PoseStamped transposed_pose_ = getTransposedRobotPose(backwards_transpose_dist); 
            check_pose_srv.request.pose = transposed_pose_;
            check_pose_srv.request.safety_dist = 0.0; // the lookahead does not need safety distance
            check_pose.check_pose_cost(local_costmap_, check_pose_srv.request, check_pose_srv.response);

            if (check_pose_srv.response.state == check_pose_srv.response.LETHAL)
            {
                // if lookahead check is lethal, check current position with safety distance
                geometry_msgs::PoseStamped current_pose_;
                local_costmap_->getRobotPose(current_pose_);
                check_pose_srv.request.pose = current_pose_;
                check_pose_srv.request.safety_dist = mvback_safety_dist;
